package mx.edu.tesoem.isc.proyectofinalmjm.DAO;
...
public class ConexionDAO {
    ...
    public boolean consultaID(String[] condicion){
        boolean estado = true;
        String [] columnas = {tabladatos.TABLA_id, tabladatos.TABLA_NOMBRE, tabladatos.TABLA_EDAD, tabladatos.TABLA_CORREO};
        String condiciones = tabladatos.TABLA_id + "= ?";
        Cursor cursor = basedatos.query(tabladatos.TABLA_NAME, columnas, condiciones, condicion,null,null,null,null);
        if (cursor.getCount()>0){
            cursor.moveToFirst();
    ...
    public boolean actualiza(ContentValues contentValues, String[] condicion){
        boolean estado = true;
        String condiciones = tabladatos.TABLA_id + " = ?";
        int afectados = (int) basedatos.update(tabladatos.TABLA_NAME, contentValues, condiciones, condicion);
        if (!(afectados>=0)) estado = false;
        return estado;
    }
    public boolean eliminar(String[] condicion){
        boolean estado = true;
        String condiciones = tabladatos.TABLA_id + " = ?";
        int afectados = (int) basedatos.delete(tabladatos.TABLA_NAME,condiciones, condicion);
        if (!(afectados>=0)) estado =false;
        return estado;
    }
